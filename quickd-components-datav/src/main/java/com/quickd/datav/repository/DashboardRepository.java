...
public interface DashboardRepository extends MongoRepository<Dashboard, String> {
	@Query(value= "{'user':?0}",fields="{'config.title':1, 'user':1, 'hash':1, 'publish.status':1, 'publish.hash':1, 'config.timestamp':1, 'publish.timestamp':1, 'imgUrl':1}")
	List<Dashboard> findByUser(String user, Pageable pageable);
	
	@Query(value= "{'publish.status':?1}",fields="{'config.title':1, 'user':1, 'hash':1, 'publish.status':1, 'publish.hash':1, 'config.timestamp':1, 'publish.timestamp':1, 'imgUrl':1}")
	List<Dashboard> findByUserAndStatus(String user, String status,Pageable pageable);
	
	@Query(value= "{'config.title':?1}",fields="{'config.title':1, 'user':1, 'hash':1, 'publish.status':1,'publish.hash':1, 'config.timestamp':1, 'publish.timestamp':1, 'imgUrl':1}")
	List<Dashboard> findByUserAndTitle(String user, String title,Pageable pageable);
	
	@Query(value= "{'config.title':?1, 'publish.status':?2}",fields="{'config.title':1, 'user':1,'hash':1, 'publish.status':1, 'publish.hash':1, 'config.timestamp':1, 'publish.timestamp':1, 'imgUrl':1}")
	List<Dashboard> findByUserAndTitleAndStatus(String user, String title, String status, Pageable pageable);
	
	@Query(value= "{'user':?0, 'project':?1}",fields="{'config.title':1, 'user':1, 'hash':1, 'project':1 ,'publish.status':1, 'publish.hash':1, 'config.timestamp':1, 'publish.timestamp':1, 'imgUrl':1}")
	List<Dashboard> findByUserAndProject(String user, String project, Pageable pageable);
	
	@Query(value= "{'publish.status':?1, 'project':?2}",fields="{'config.title':1, 'user':1, 'hash':1, 'project':1 ,'publish.status':1, 'publish.hash':1, 'config.timestamp':1, 'publish.timestamp':1, 'imgUrl':1}")
	List<Dashboard> findByUserAndStatusAndProject(String user, String status, String project,Pageable pageable);
	
	@Query(value= "{'config.title':?1, 'project':?2}",fields="{'config.title':1, 'user':1, 'hash':1, 'project':1 ,'publish.status':1,'publish.hash':1, 'config.timestamp':1, 'publish.timestamp':1, 'imgUrl':1}")
	List<Dashboard> findByUserAndTitleAndProject(String user, String title, String project, Pageable pageable);
	
	@Query(value= "{'config.title':?1, 'publish.status':?2, 'project':?3}",fields="{'config.title':1, 'user':1,'hash':1,'project':1 , 'publish.status':1, 'publish.hash':1, 'config.timestamp':1, 'publish.timestamp':1, 'imgUrl':1}")
