...
import com.quickd.online.graphreport.dto.OnlGraphreportHeadDto;
...
public class OnlGraphreportHeadController extends BaseController<IOnlGraphreportHeadService, OnlGraphreportHead, OnlGraphreportHeadDto> {
    ...
    public JsonResult< List<OnlGraphreportItem>> getDetailInfo(@RequestParam(name = "id", required = true) String id) {
        JsonResult< List<OnlGraphreportItem>> result = new JsonResult<>();
        List<OnlGraphreportItem> onlGraphreportItemList = onlGraphreportItemService.selectByMainId(id);
        result.ok(onlGraphreportItemList);
        return result;
    }
    /**
     * 修改
